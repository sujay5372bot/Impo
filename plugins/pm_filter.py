        ...
        btn = [
            [
                InlineKeyboardButton(
                    text=f"<b>📮 {get_size(file.file_size)} 🏹 {' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@') and not x.startswith('www.'), file.file_name.split()))}\n\n</b>", callback_data=f'{pre}#{file.file_id}'
                ),
            ]
            for file in files
        ]
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
        ...
        try:
            ...
            return
    # don't change anything without contacting me @kingvj0
...
async def auto_filter(client, name, msg, reply_msg, ai_search, spoll=False):
    ...
    if imdb:
        ...
        if not settings["button"]:
            ...
            for file in files:
                cap += f"<b>📮 {get_size(file.file_size)} 🏹 {' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@') and not x.startswith('www.'), file.file_name.split()))}\n</b>"
    else:
        if settings["button"]:
